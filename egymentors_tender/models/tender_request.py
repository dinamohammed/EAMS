...
class TenderRequest(models.Model):
    ...
    state = fields.Selection(_STATES, string='Status', readonly=True, index=True, copy=False, default='draft',
                             tracking=True)
    request_line = fields.One2many('tender.request.line', 'request_id', string='Request Lines', readonly=1,
                                   states=READONLY_STATES, copy=True)
    notes = fields.Text("Notes", placeholder="Write Notes")
    tender_responsible_ids = fields.One2many(comodel_name='res.users', inverse_name='tender_request_id',
                                             string="Responsible")
    ...
    def print_request(self):
        return self.env.ref('egymentors_tender.tender_request_report').report_action(self)
...
class TenderRequestLine(models.Model):
    ...
    request_id = fields.Many2one(comodel_name='tender.request', string='Request Reference', index=True, required=True,
                                 ondelete='cascade')
    state = fields.Selection(_STATES, string='Status', readonly=True, index=True, copy=False, default='draft',
                             tracking=True)
