...
class PurchaseOrderInherit(models.Model):
    ...
    request_id = fields.Many2one('tender.request', 'Tender Request', copy=False, domain=[('state', '=', 'done')],
                                 help="Choose Confirmed request and load it's lines on PO")
    request_ids = fields.Many2many(comodel_name='tender.request', copy=False)
        ...
        for order in self:
            ...
            if request_ids:
                ...
                for request_id in request_ids:
                    ...
                    # 	raise ValidationError(_("This Request %s Used Before in Order %s") % (request_id.name,
                ...
                raise ValidationError(_("No Request To load"))
    ...
    def remove_pr_lines(self):
        for order in self:
            ...
            if request_ids:
                    ...
                    for line in request_id.request_line:
                        ...
                        if order_line:
                            ...
                            # THis line is edit and qty is bellow pr line qty (User miss using)
                                ...
                                raise ValidationError(_("User Changed Qty for this product [%s]\n"
                ...
                raise ValidationError(_("No Request To remove"))
    ...
    def _get_tax_key_for_group_add_base(self, line):
        """
        Useful for compute_tender_taxes_by_group
...
class PurchaseOrderLineInherit(models.Model):
    ...
    request_line_ids = fields.Many2many('tender.request.line', string="Request Lines")
    requisition_line_id = fields.Many2one('purchase.requisition.line', 'Requisition Line')
    accepted = fields.Boolean("Accepted")
    accepted_tender = fields.Boolean("Accepted Tender")

    def unlink(self):
        for line in self:
            if not line.request_line_ids and line.order_id.state in ['purchase', 'done']:
                raise UserError(_('Cannot delete a tender order line which is in state \'%s\'.') % (line.state,))
        ...
        for line in obj:
            if line.accepted or line.accepted_tender:
                ...
                for l in other_lines:
                    raise ValidationError(_("You Can't accept this line: %s\nIt's already accepted on "
    ...
    def create(self, vals):
        ...
        if ('accepted' or 'accepted_tender') in vals and line.request_line_ids:
            for request_line in line.request_line_ids:
                print("REQUEST LINE: ", request_line)
                if vals.get('accepted') or vals.get('accepted_tender'):
                    state = 'locked'
                else:
                    continue
    ...
    def write(self, vals):
        ...
        if ('accepted' or 'accepted_tender') in vals and self.request_line_ids:
