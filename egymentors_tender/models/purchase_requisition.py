...
class PurchaseRequisitionInherit(models.Model):
    ...
    request_id = fields.Many2one('tender.request', 'Tender Request', copy=False,
                                 # domain=_get_request_domain,
                                 # domain=lambda r: r.state in ['done'] and r.tender_responsible_ids.ids in r.env.user.id,
                                 domain=lambda self: "[('state', 'in', ['done']), \
                                                       ('tender_responsible_ids', 'in', [%s])]" % self.env.user.id,
    ...
    signatures_tender_ids = fields.Many2many(comodel_name='agreement.signature.tender',
                                             help="Add signatures to be added in printouts")
    line_ids = fields.One2many('purchase.requisition.line', 'requisition_id', string='Products to Tender',
    ...
    def write(self, vals):
        if vals.get('request_id'):
            vals['origin'] = self.env['tender.request'].browse(vals.get('request_id')).name
        ...
        for rec in self:
            ...
            if request_ids:
                ...
                for request_id in request_ids:
                    ...
                    # 	raise ValidationError(_("This Request %s Used Before in requisition %s") % (request_id.name,
                    # 																		rec.name))
...
class PurchaseRequisitionLineInherit(models.Model):
    ...
    request_line_ids = fields.Many2many('tender.request.line', string="Request Line")

    def prepare_tender_order_line(self, name, product_qty=0.0, price_unit=0.0, taxes_ids=False):
        res = super(PurchaseRequisitionLineInherit, self).prepare_tender_order_line(name, product_qty, price_unit,
                                                                                    taxes_ids)
