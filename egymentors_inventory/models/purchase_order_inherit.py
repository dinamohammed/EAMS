...
class PurchaseOrderInherit(models.Model):
    ...
    tender_request_id = fields.Many2one(comodel_name='tender.request', string="Purchase Request", copy=False,
                                        domain=[('state', '=', 'done')],
                                        help="Choose Confirmed tender request and load it's lines on PO")
    ...
    def load_tr_lines(self):
        for order in self:
            if order.tender_multi_requisition:
                tender_request_ids = order.tender_request_ids
            else:
                tender_request_ids = [order.tender_request_id]
            if tender_request_ids:
                order_products = order.order_line.mapped('product_id.id')
                for tender_request_id in tender_request_ids:
                    # if tender_request_id.purchase_order_id:
                    # 	raise Warning(_("This Tender Request %s Used Before in Order %s") % (tender_request_id.name,
                    # 																  tender_request_id.purchase_order_id.name))
                    for line in tender_request_id.tender_request_line.filtered(lambda l: l.state == 'done'):
                        if line.product_id.id in order_products:
                            order_line = order.order_line.filtered(lambda l:
                                                                   l.product_id.id == line.product_id.id)
                            tender_request_line_ids = order_line[0].tender_request_line_ids.ids
                            ...
                            order_products.append(line.product_id.id)
                    # 'state': 'locked',
                    tender_request_id.write({'purchase_order_id': order.id})
                order.tender_request_added = True
            else:
                raise Warning(_("No Tender Request To load"))
            ...
            if tender_request_ids:
                for tender_request_id in tender_request_ids:
                    for line in tender_request_id.tender_request_line:
                        order_line = order.order_line.filtered(lambda l: line.id in l.tender_request_line_ids.ids)
                        if order_line:
                            ...
                            # IT's Order Line And will deduct tr line qty
                                ...
                                tender_request_line_list = order_line[0].tender_request_line_ids.ids
                            ...
                            # THis line is edit and qty is bellow pr line qty (User miss using)
                                ...
                                raise Warning(_("User Changed Qty for this product [%s]\n"
                                                " Please Make it's qty bigger than tender request line qty:%s") % (
                                                  order_line[0].product_id.display_name, line.product_qty
                                              ))
                    tender_request_id.write({'state': 'done', 'purchase_order_id': False})
                order.tender_request_added = False
            else:
                raise Warning(_("No Tender Request To remove"))
    ...
    def _onchange_requisition_id(self):
        ...
        self.tender_request_id = self.requisition_id.tender_request_id and self.requisition_id.tender_request_id.id or False

        self.request_ids = self.requisition_id.request_ids and [(6, 0, self.requisition_id.request_id.ids)] or False
        self.tender_request_ids = self.requisition_id.tender_request_ids and [
            (6, 0, self.requisition_id.tender_request_id.ids)] or False
...
class PurchaseOrderLineInherit(models.Model):
    ...
    tender_request_line_ids = fields.Many2many('tender.request.line', string="Tender Request Lines")
    requisition_line_id = fields.Many2one('purchase.requisition.line', 'Requisition Line')
    accepted = fields.Boolean("Accepted")
    accepted_tender = fields.Boolean("Tender Accepted")

    def unlink(self):
        for line in self:
            if not line.request_line_ids and line.order_id.state in ['purchase', 'done']:
                raise UserError(_('Cannot delete a purchase order line which is in state \'%s\'.') % (line.state,))
            if not line.tender_request_line_ids and line.order_id.state in ['purchase', 'done']:
                raise UserError(_('Cannot delete a purchase order line which is in state \'%s\'.') % (line.state,))
        return super(models.Model, self).unlink()

    @api.constrains('requisition_line_id', 'accepted', 'accepted_tender')
    def _accepted_lines_constrain(self):
        obj = self.env['purchase.order.line']
        for line in obj:
            if line.accepted or line.accepted_tender:
                other_lines = obj.search(['|', ('accepted', '=', True), ('accepted_tender', '=', True),
                                          ('id', '!=', line.id),
    ...
    def create(self, vals):
        ...
        # Tender Request:
        print("VALS: ", vals.get('accepted_tender'), line.tender_request_line_ids)
        if 'accepted_tender' in vals and line.tender_request_line_ids:
            for tender_request_line in line.tender_request_line_ids:
                print("Tender REQUEST LINE: ", tender_request_line)
                if vals.get('accepted_tender'):
                    state = 'locked'
                else:
                    continue
                tender_request_line.write({'state': state})
                tender_request_id = tender_request_line.tender_request_id
                if all(state == 'locked' for state in tender_request_id.tender_request_line.mapped('state')):
                    tender_request_id.write({'state': 'locked'})
                elif any(state == 'done' for state in tender_request_id.tender_request_line.mapped('state')) \
                        and tender_request_id.state == 'locked':
                    tender_request_line.write({'state': 'done'})
        return line
    ...
    def write(self, vals):
        ...
        # Tender Request:
        if 'accepted_tender' in vals and self.tender_request_line_ids:
            for tender_request_line in self.tender_request_line_ids:
                print("Tender REQUEST LINE: ", tender_request_line)
                if vals.get('accepted_tender'):
                    state = 'locked'
                else:
                ...
                if all(state == 'locked' for state in tender_request_id.tender_request_line.mapped('state')):
                    tender_request_id.write({'state': 'locked'})
                        ...
                        and tender_request_id.state == 'locked':
                    tender_request_id.write({'state': 'done'})
