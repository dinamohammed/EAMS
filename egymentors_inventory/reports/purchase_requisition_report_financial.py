...
class PartnerXlsx(models.AbstractModel):
    _name = 'report.egymentors_inventory.report_purchase_req_financial'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        for obj in partners:
            ...
            if vendors_dict:
                row += 2
                worksheet.merge_range(row, 0, row, 2, "رأى اللجنه الماليه", cell_format_right_bold)
                row += 1
                for vendor_line in vendors_dict:
                ...
                row += 2
                # Signatures
                worksheet.merge_range(row, 0, row, 1, "توقيعات اعضاء اللجنه المالية", cell_format_right_bold)
