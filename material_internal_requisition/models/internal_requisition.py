...
class MaterialInternalRequisition(models.Model):
    ...
    requisition_line_ids = fields.One2many('material.internal.requisition.line', 'requisition_id',
    ...
    note = fields.Text(string='Reason for Requisition', required=False, copy=True)
    ...
    def _get_internal_picking_count(self):
        for requisition in self:
            requisition.internal_picking_count = self.env['stock.picking'].sudo().search_count(
    ...
    def unlink(self):
        for rec in self:
            if rec.state not in ('draft', 'cancel', 'reject'):
                raise Warning(
                    _('You cannot delete an internal requisition unless the status of the request is new, canceled, or rejected.'))
    ...
    def requisition_confirm(self):
        for rec in self:
            email_template_to_department_manager = self.env.ref(
                'material_internal_requisition.email_template_to_department_manager')
